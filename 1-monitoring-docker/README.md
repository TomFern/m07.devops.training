...
4. Once started, go to `http://localhost:9090` you should be seeing the prometheus dashboard
    - Try the following query: `request_processing_seconds_count`
...
7. Select prometheus
8. On connection, type: `http://prometheus:9090`. Note that you can access other Docker containers using the `service` defined on `compose.yml`
...
11. Add a pane
12. Select prometheus as the data source
13. Under metric, select "request_processing_seconds_sum". This is the data the metrics_app.py application supplies.
...
16. When done, press `docker compose down -v` to shutdown the system
